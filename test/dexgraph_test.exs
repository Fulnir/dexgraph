defmodule DexGraphTest do
  ...
  setup_all do
      IO.puts "Seneca config server: #{Application.get_env(:seneca, :server)}"
      # Komplette Datenbank löschen
      alter("{\"drop_all\": true}")
      # Schema
      write_schema(@testing_schema)
      # Daten
      result = write_testing_data(@testing_data)
      :ok
  end

  test "Query unknown node" do
      {result_atom, node} = query_node("name", "test_dummy")
  ...
  test "Add node" do
      {:ok, node} = mutate_node("name", "Edwin Bühler")
      Logger.warn node["uids"]["identifier"]
      ...
      {:ok, node} = mutate_node(identifier, "id", "EdwinBühler")
      {:ok, node} = query_node("name", "Edwin Bühler")
      assert "Edwin Bühler" == node["name"]
